...
async function loadElevationChart(trail) {
  ...
  try {
    if (!trail.geometry || !trail.geometry.length) {
    ...
    const allPoints = [];
    for (let s = 0; s < trail.geometry.length; s++) {
      const seg = trail.geometry[s];
      if (!seg) continue;
    ...
    if (!(totalKm > 0)) throw new Error("zero length path");

    const SAMPLES = 12;
    const sampled = [];
    for (let i = 0; i < SAMPLES; i++) {
      const targetDist = (i / (SAMPLES - 1)) * totalKm;
      let idx = cum.findIndex(function (d) {
        return d >= targetDist;
      });
    ...
    for (var si = 0; si < sampled.length; si++) {
      locParts.push(sampled[si].point[0] + "," + sampled[si].point[1]);
    }
    var locString = locParts.join("|");

    const resp = await fetch(
      "/api/elevation?locations=" + encodeURIComponent(locString)
    );
    const data = await resp.json();
    ...
    if (!elevations || !elevations.length) {
      throw new Error("empty elevations");
    }

    var known = 0;
    for (var ki = 0; ki < elevations.length; ki++) {
      if (elevations[ki] !== null && elevations[ki] !== undefined) known++;
    }
    if (known < 2) throw new Error("not enough elevation data (" + known + ")");

    var gainFt = 0;
    for (var gi = 1; gi < elevations.length; gi++) {
      if (
        elevations[gi] != null &&
        elevations[gi - 1] != null &&
        elevations[gi] > elevations[gi - 1]
      ) {
        gainFt += elevations[gi] - elevations[gi - 1];
      }
    }
    ...
    var labels = [];
    for (var li = 0; li < sampled.length; li++) {
      labels.push(sampled[li].mi.toFixed(2));
    }
      ...
      options: {
        ...
        onHover: function (event, elements) {
          if (!elements || !elements.length) {
            if (readout) readout.textContent = "Drag to explore";
            return;
          }
          var idx = elements[0].index;
          var mi = sampled[idx].mi.toFixed(1);
          var ft = elevations[idx];
          if (readout) {
            readout.textContent =
              ft == null
                ? mi + " mi"
                : mi + " mi · " + Math.round(ft).toLocaleString() + " ft";
          }
        },
      },
    });
    ...
    note.textContent =
      "Approx. " + Math.round(gainFt) + " ft elevation gain, estimated from sampled points.";
  } catch (err) {
    console.error("elevation chart:", err);
    note.textContent =
      "Elevation unavailable: " + (err && err.message ? err.message : "unknown error");
  }
          }
  const note = document.getElementById("elevationNote");
  try {
